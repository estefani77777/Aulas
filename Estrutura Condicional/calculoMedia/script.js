function  CalcularMedia (){
    ...
    if (isNaN(nota1) || isNaN(nota2) || isNaN(nota3)){
        ...
        if (media>7) {
           resultado.innerHTML = `media: ${Media.toFixed (2)} - Situacao Aprovado`
        }

    else if (media >= 5){
            resultado.innerHTML = `media: ${Media.toFixed (2)} - Situacao Recuperação`
        }    

    else {
            resultado.innerHTML = `media: ${Media.toFixed (2)} - Situacao Reprovado´
        }    
